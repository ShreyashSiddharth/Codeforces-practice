...
i=n-1;
line(maxx+c[i][0],maxy-c[i][1],maxx+c[0][0],maxy-c[0][1]); delay(1000);
//draw X-Y axis
setcolor(WHITE);
...
for(j=0;j<3;j++)
b[i][j]=0;
A=(ra*3.14)/180; b[0][0]=b[1][1]=cos(A);
b[0][1]=sin(A);
b[1][0]=-sin(A); b[2][0]=(-xp * cos(A)) +
(yp * sin(A)) + xp; b[2][1]=(-xp * sin(A)) - (yp *
cos(A)) + yp;
b[2][2]=1;
mult(n,b,c,a);
setcolor(YELLOW); display(n,a);
...
initgraph(&gd,&gm,NULL); cout<<"\nEnter thenumber of vertices: "; cin>>n;
for(i=0;i<n;i++)
{
cout<<"Enter the co-ordinates of vertex "<<i+1<<":";
cin>>c[i][0]>>c[i][1]; c[i][2]=1;
}
do
{
cout<<"\n\n------------2-D Transformation-------------";
...
cleardevice();
setcolor(CYAN);
display(n,c); scaling(n,c,sx,sy);
break;
case 3: cout<<"\nEnter the angle for rotation: ";
cin>>ra;
cleardevice();
setcolor(CYAN);
display(n,c); rotation(n,c,ra);
break;
